...
public class CustomersController : ControllerBase
{
    ...
    public async Task<ActionResult<List<CustomerDto>>> Get(CancellationToken ct)
    {
        ...
        return await _db.Customers.AsNoTracking().Where(x => x.IsActive).OrderBy(x => x.Name)
            .Select(x => new CustomerDto(x.Id, x.Name, x.Phone, x.Email, x.Area, x.Balance, x.IsActive, x.IsCompany))
            .ToListAsync(ct);
    }
    ...
    public async Task<ActionResult<CustomerDto>> Create(UpsertCustomerRequest req, CancellationToken ct)
    {
        ...
        var c = new Customer
        {
            ...
            Balance = req.Balance, IsActive = req.IsActive, IsCompany = req.IsCompany, CreatedAtUtc = DateTime.UtcNow, UpdatedAtUtc = DateTime.UtcNow
        };
        _db.Customers.Add(c);
        await _db.SaveChangesAsync(ct);
        return Created($"/api/customers/{c.Id}", new CustomerDto(c.Id, c.Name, c.Phone, c.Email, c.Area, c.Balance, c.IsActive, c.IsCompany));
    }
    ...
    public async Task<ActionResult<CustomerDto>> Update(Guid id, UpsertCustomerRequest req, CancellationToken ct)
    {
        ...
        c.Balance = req.Balance; c.IsActive = req.IsActive; c.IsCompany = req.IsCompany; c.UpdatedAtUtc = DateTime.UtcNow;
        await _db.SaveChangesAsync(ct);
        return new CustomerDto(c.Id, c.Name, c.Phone, c.Email, c.Area, c.Balance, c.IsActive, c.IsCompany);
    }
